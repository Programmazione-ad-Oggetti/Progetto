...
public class JsonParser {
	...
	public ArrayList<Tweet> parsingDataset() throws ParseException{
		...
		for(int i = 0; i < parser.size(); i++) {
			...
			tweet.setData((String) parsedTweet.get("created at")); //Viene presa la data
			tweet.setID((long) parsedTweet.get("created at"));
			
			Hashtag hashtg;
			
			//Creazione dell'oggetto entities che permette di effetttuare la get() dell'hashtag, 
			//in quanto (nel JSON) il campo Hashtag è inserito dentro l'oggetto "entities"
			...
			//in quanto (nel JSON) il campo "favorite_count" è inserito dentro l'oggetto "retweeted_status"
